...
def test_parallel_embedding(tensor_model_parallel_size):
    ...
    input_data = torch.LongTensor(
        size=(batch_size, seq_length)).random_(0, vocab_size).to(mg_accelerator.device())
    loss_weight = torch.randn([batch_size, seq_length, hidden_size]).to(mg_accelerator.device())

    set_random_seed(seed)
    embedding_original = torch.nn.Embedding(vocab_size, hidden_size).to(mg_accelerator.device())
    ...
    embedding_parallel = layers.ParallelEmbedding(
        vocab_size, hidden_size, init_method=init.normal_).to(mg_accelerator.device())
...
def test_column_parallel_linear(tensor_model_parallel_size):
    ...
    # Network
    identity_layer = IdentityLayer2D(batch_size, input_size).to(mg_accelerator.device())
    linear_layer = mpu.ColumnParallelLinear(
        input_size, output_size, keep_master_weight_for_test=True).to(mg_accelerator.device())
    loss_weight = torch.randn([batch_size, output_size]).to(mg_accelerator.device())
    # Forward
    ...
    A = linear_layer.master_weight.to(mg_accelerator.device())
    dLdA = torch.matmul(dLdY.t(), X)
    dLdb = torch.matmul(torch.ones(batch_size, 1).to(mg_accelerator.device()).t(), dLdY).view(-1)
...
def parallel_self_attention(tensor_model_parallel_size, num_att_heads_per_partition,
    ...
    identity_layer = IdentityLayer3D(batch_size, sequence_length,
                                     hidden_size).to(mg_accelerator.device())
    attention_layer = mpu.BertParallelSelfAttention(hidden_size, num_att_heads,
                                                    dropout_prob).to(mg_accelerator.device())
    loss_weight = torch.randn([batch_size, sequence_length, hidden_size]).to(mg_accelerator.device())
    attention_mask = torch.randn([batch_size, 1, 1, sequence_length]).to(mg_accelerator.device())
    # Forward
    input_ = identity_layer()
...
def parallel_transformer(tensor_model_parallel_size, num_att_heads_per_partition,
    ...
    transformer_layer = mpu.BertParallelTransformerLayer(
        ...
        torch.nn.functional.relu, 1.0e-5).to(mg_accelerator.device())
