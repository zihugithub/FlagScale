...
def test_fused_softmax():
    bert = BertModel.from_pretrained("bert-base-cased").to(mg_accelerator.device()).half()
    ...
    embedding_output = bert.embeddings(
        input_ids=tokens["input_ids"].to(mg_accelerator.device()),
        position_ids=None,
        token_type_ids=tokens["token_type_ids"].to(mg_accelerator.device()),
        inputs_embeds=None,
    ...
    mask = bert.get_extended_attention_mask(
        attention_mask=tokens["attention_mask"].to(mg_accelerator.device()),
...
def test_fused_upper_triangle_mask_softmax():
    gpt = GPT2Model.from_pretrained("gpt2").to(mg_accelerator.device()).half()
    ...
    hidden_states = gpt.wte(tokens["input_ids"].to(mg_accelerator.device()))
...
def test_layer_norm():
    ...
    fused_layernorm_layer = (
        MixedFusedLayerNorm(normalized_shape=embedding_output.size(-1)).to(mg_accelerator.device()).half()
    )

    torch_layernorm_layer = (
        LayerNorm(normalized_shape=embedding_output.size(-1)).to(mg_accelerator.device()).half()
    )
...
def test_masked_softmax_forward():
    ...
    for qlen in [128, 256, 1024, 2048, 4096]:
        for klen in [128, 256, 1024, 2048]:
            inputs = torch.normal(0, 2, (batch, attn, qlen, klen), dtype=torch.float16, device=mg_accelerator.device(0))
            masks = torch.randint(0, 2, (batch, 1, qlen, klen), dtype=torch.bool, device=mg_accelerator.device(0))
...
def test_masked_softmax_backward():
    ...
    for qlen in [128, 256, 1024, 2048, 4096]:
        for klen in [128, 256, 1024, 2048]:
            inputs = torch.normal(0, 2, (batch, attn, qlen, klen), dtype=torch.float16, device=mg_accelerator.device(0))
            backward = torch.rand_like(inputs, dtype=torch.float16, device=mg_accelerator.device(0))
            masks = torch.randint(0, 2, (batch, 1, qlen, klen), dtype=torch.bool, device=mg_accelerator.device(0))
            softmax_results = scaled_masked_softmax_cuda.forward(inputs, masks, scale_t[0].item())
...
def test_allmasked_softmax_forward():
    ...
    for qlen in [128, 256, 1024, 2048, 4096]:
        for klen in [128, 256, 1024, 2048]:
            inputs = torch.normal(0, 2, (batch, attn, qlen, klen), dtype=torch.float16, device=mg_accelerator.device(0))
            masks = torch.ones((batch, 1, qlen, klen), dtype=torch.bool, device=mg_accelerator.device(0))
            softmax_results = scaled_masked_softmax_cuda.forward(inputs, masks, scale_t[0].item())
