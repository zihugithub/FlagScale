...
def test_inference_equal(path, task, model, case, platform, device):
    ...
    Gold file format: results_gold/<case>.json
    Structure: { "<platform>": { "<device>": ["line1", "line2", ...] } }
    """
    # Locate the inference output log file
    test_result_path = os.path.join(path, task, model, "test_results", case)
    ...
    # Load JSON gold file and extract platform/device-specific values
    ...
    assert platform and platform != "none", "Gold file is platform-classified but --platform not specified"
    assert platform in gold_json, f"Platform '{platform}' not found in gold file. Available: {list(gold_json.keys())}"
    platform_data = gold_json[platform]

    assert device and device != "none", "Gold file is device-classified but --device not specified"
    assert device in platform_data, f"Device '{device}' not found in gold file under '{platform}'. Available: {list(platform_data.keys())}"
    gold_value_lines = platform_data[device]
