...
class test_time_statistics(object):
    ...
    def get_cpuANDgpu(self):
        ...
        cpuANDgpu = "gpu_model :{}\ngpu_processor :{}\ncpu_model :{}\ncpu_processor :{}".format(gpu_models,gpu_processor,cpu_model,cpu_processor)
        return cpuANDgpu
    ...
    def create_unit_layout(self):
        ...
        ws["A{}".format(flagscale_end_index+1)] = "CPU | GPU 型号与数量："
    ...
    def create_functional_layout(self):
        ...
        # The quantity and model of CPUs and GPUs
        ws.merge_cells("A{}:C{}".format(serve_end_index+1,serve_end_index+1))
        ws["A{}".format(serve_start_index+1)] = "CPU | GPU 型号与数量："
    ...
    def functional_append_data(self):
        # wb = load_workbook(f"{self.xlsx_path()}/{self.file_name_xlsx}")
        # ws = wb.active
        with open(self.data_file(),"r") as f:
            lines = f.readlines()
            cases_runtime = [] # ['inference-pipeline_Qwen3-4B_nvidia: 77', 'inference-pipeline_Qwen3-4B_nvidia_flaggems: 90']
            for line in lines:
                if "runtime" in line:
                    runtime = "".join(f"{line.split(r' ')[1].strip()}: {line.split(r' ')[3].strip()}")
                    cases_runtime.append(runtime)
        ...
        #     print(merged_cells)
            ...
            #     for row in rows:
            #         if row.value == "inference-pipeline":
            #             print(row)
